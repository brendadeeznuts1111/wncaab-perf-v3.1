...
export interface StaticFileConfig {
  ...
  file: string;
  /** Whether file is immutable (affects Cache-Control header and route strategy) */
  immutable?: boolean;
  /** 
   * Whether to use static route (buffered at startup) vs file route (read per request)
   * 
   * Static Route (true):
   *   - Zero filesystem I/O during requests (content served from memory)
   *   - ETag support (automatically generated/validated by Bun)
   *   - If-None-Match → 304 Not Modified
   *   - No 404 handling (startup errors if file missing)
   *   - Full file content in RAM
   *   - ~15% performance improvement
   *   - Best for: Small static assets, frequently accessed files
   * 
   * File Route (false):
   *   - Filesystem reads per request
   *   - Built-in 404 handling
   *   - Last-Modified support, If-Modified-Since → 304 Not Modified
   *   - Range request support (Content-Range headers)
   *   - Streaming transfers (memory efficient, buffers small chunks)
   *   - Best for: Large files, dynamic content, files that change frequently
   * 
   * Auto-detect (undefined):
   *   - immutable=true → static route
   *   - immutable=false/undefined → file route
   */
    ...
    STATIC_FILES.map(async ({ path, file, immutable, useStaticRoute }) => {
      ...
      if (shouldUseStaticRoute) {
        ...
        try {
          ...
          // ✅ ETag computed once at startup (cached per file)
          const etag = await bunFile.etag();
          // ✅ Content-Length prevents chunked encoding (faster transfers)
...
export function generateStaticRouteTags(): string[] {
  return STATIC_FILES.map(({ path }) => {
    ...
    const normalized = path
      .replace(PATH_PREFIX_REGEX, '')  // Remove leading slash
      .replace(PATH_SEPARATOR_REGEX, '-')  // Replace slashes with dashes
      .replace(DOT_REGEX, '-');  // Replace dots with dashes
