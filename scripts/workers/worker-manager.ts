...
class ResilientWorkerManager {
  ...
  private workerStates: Map<string, { status: string; createdAt: number; errorCount: number }> =
    new Map();
  ...
  private startEventLoopMonitoring(): void {
    ...
    const measureTick = () => {
      ...
      this.eventLoopMetrics.longTickRatio =
        this.eventLoopMetrics.tickCount > 0
  ...
  createWorker(id: string, env?: Record<string, string>): Worker {
    ...
    // Note: setEnvironmentData is synchronous in Bun 1.3
    setEnvironmentData('tes-worker-config', {
      workerId: id,
      registry: true,
      ...env, // Merge additional env vars into config
    });
  ...
  private async handleWorkerError(workerId: string, error: ErrorEvent): Promise<void> {
    ...
    const delay = Math.min(
      respawnInfo.nextRespawnDelay * 2 ** (respawnInfo.attemptCount - 1),
 ...
 */
export async function handleWorkerScale(
  req: Request,
 ...
 */
export async function handleWorkerSnapshot(
  req: Request,
  options: { id: string },
): Promise<Response> {
  const { id } = options;
  ...
  try {
    ...
    return new Response(
      JSON.stringify({
        ...
        message:
          'Worker Telemetry API is not running. Start it with: bun run scripts/worker-telemetry-api.ts',
        workerId: id,
      }),
      {
        status: 503,
